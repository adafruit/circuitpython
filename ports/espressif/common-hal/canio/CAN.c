...
static twai_handle_t *const twai_insts[SOC_TWAI_CONTROLLER_NUM] = { &twai_bus_0 };

#elif SOC_TWAI_CONTROLLER_NUM == 2
static twai_handle_t twai_bus_0;
static twai_handle_t twai_bus_1;
static twai_handle_t *const twai_insts[SOC_TWAI_CONTROLLER_NUM] = { &twai_bus_0, &twai_bus_1 };
#endif
...
#define DIV_ROUND_UP(a, b) (((a) + (b) - 1) / (b))
    if (reserved_can_instance == (1 << SOC_TWAI_CONTROLLER_NUM) - 1) {
        /* 3 controller all taken => reserved_can_instance = 7
        *  1 << 3 is 8
        */
        mp_raise_ValueError(MP_ERROR_TEXT("All CAN peripherals are in use"));
    } else if (reserved_can_instance > SOC_TWAI_CONTROLLER_NUM) {
        mp_raise_ValueError(MP_ERROR_TEXT("This cpu only supports 1 CAN peropheral"));
    }
    ...
    /* reserved_can_instance lowest significant bit at 0 is available */
    while (!(reserved_can_instance & (1 << this_can_instance))) {
        this_can_instance++;
    }
    reserved_can_instance |= (1 << this_can_instance);
