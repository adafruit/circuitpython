...
The File Transfer transfer characteristic is encrypted (ENC_NO_MITM), so this
test requires pairing. nrf54lm20bsim LE encryption is not yet functional in
bsim, so this test is restricted to native_nrf5340bsim (matching
test_bsim_ble_pairing.py). Real hardware works on both.
"""
