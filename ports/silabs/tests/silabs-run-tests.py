...
class report:
    ...
    header_cell_format1 = workbook.add_format(
        {
            "align": "center",
            "valign": "vcenter",
            "border": 1,
    ...
    pass_cell_format1 = workbook.add_format(
        {"align": "center", "valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "green"}
    )
    pass_cell_format2 = workbook.add_format(
        {"valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "green"}
    )
    fail_cell_format1 = workbook.add_format(
        {"align": "center", "valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "red"}
    )
    fail_cell_format2 = workbook.add_format(
        {"valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "red"}
    )
    skip_cell_format1 = workbook.add_format(
        {"align": "center", "valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "yellow"}
    )
    skip_cell_format2 = workbook.add_format(
        {"valign": "vcenter", "border": 1, "text_wrap": 1, "bg_color": "yellow"}
    )
    ...
    def __init__(self, filename):
        ...
        if os.path.exists("pyboard.py"):
            os.remove("pyboard.py")

    def write_header(self, row, column):
        self.start_row = row
        self.start_column = column
        cell_format = self.header_cell_format1

        self.worksheet.write(row, column, "Number", cell_format)
        self.worksheet.write(row, column + 1, "Test Case", cell_format)
        self.worksheet.write(row, column + 2, "Result", cell_format)
        self.worksheet.write(row, column + 3, "Note", cell_format)

        self.worksheet.set_column(column, column, 8)
        self.worksheet.set_column(column + 1, column + 1, 40)
        self.worksheet.set_column(column + 2, column + 2, 10)
        self.worksheet.set_column(column + 3, column + 3, 50)

    def write_line(self, test_name, result, note=""):

        cell_format1 = None
        cell_format2 = None

        if result.lower() == "pass":
            cell_format1 = self.pass_cell_format1
            cell_format2 = self.pass_cell_format2
        elif result.lower() == "fail":
            cell_format1 = self.fail_cell_format1
            cell_format2 = self.fail_cell_format2
        elif result.lower() == "skip":
            cell_format1 = self.skip_cell_format1
            cell_format2 = self.skip_cell_format2
        else:
            raise Exception("Invalid result")

        self.test_number += 1
        self.worksheet.write(
            self.start_row + self.test_number, self.start_column, self.test_number, cell_format1
        )
        self.worksheet.write(
            self.start_row + self.test_number, self.start_column + 2, result, cell_format2
        )
        self.worksheet.write(
            self.start_row + self.test_number, self.start_column + 1, test_name, cell_format2
        )
        self.worksheet.write(
            self.start_row + self.test_number, self.start_column + 3, note, cell_format2
        )

    def write_summary(self, num_pass, num_fail, num_skip):

        self.worksheet.write(
            self.start_row, self.start_column + 6, "TOTAL", self.header_cell_format1
        )
        self.worksheet.write(
            self.start_row,
            self.start_column + 7,
            num_pass + num_fail + num_skip,
            self.header_cell_format1,
        )

        self.worksheet.write(
            self.start_row + 1, self.start_column + 6, "PASS", self.pass_cell_format1
        )
        self.worksheet.write(
            self.start_row + 1, self.start_column + 7, num_pass, self.pass_cell_format1
        )

        self.worksheet.write(
            self.start_row + 2, self.start_column + 6, "FAIL", self.fail_cell_format1
        )
        self.worksheet.write(
            self.start_row + 2, self.start_column + 7, num_fail, self.fail_cell_format1
        )

        self.worksheet.write(
            self.start_row + 3, self.start_column + 6, "SKIP", self.skip_cell_format1
        )
        self.worksheet.write(
            self.start_row + 3, self.start_column + 7, num_skip, self.skip_cell_format1
        )
...
def run_tests(pyb, tests, args, result_dir, num_threads=1):
    ...
    def run_one_test(test_file):
        ...
        if skip_it:
            ...
            Report.write_line(test_file, "SKIP")
            return
        ...
        if output_mupy == b"SKIP\n":
            print("skip ", test_file)
            skipped_tests.append(test_name)
            Report.write_line(test_name, "SKIP")
            return
        ...
        if output_expected == output_mupy:
            ...
            Report.write_line(test_basename, "PASS")
        else:
            ...
            Report.write_line(test_basename, "FAIL", "'" + output_mupy.decode("utf-8"))
    ...
    Report.write_summary(
        test_count.value + len(skipped_tests), passed_count.value, len(skipped_tests)
    )
    Report.close_report()
...
""",
    ...
    EXTERNAL_TARGETS = ("pyboard", "wipy", "esp8266", "esp32", "minimal", "nrf", "efr32")
