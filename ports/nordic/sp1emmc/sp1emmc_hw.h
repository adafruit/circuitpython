...
// The hardware sp1emmc.c drives: the five pins, the RTC2 tick source and the
// SPIM3 data engine.
...
// Pins (SP-1 wiring).
#define SP1EMMC_PIN_CLK   6u    // P0.06
#define SP1EMMC_PIN_DAT0  7u    // P0.07
#define SP1EMMC_PIN_CMD   8u    // P0.08
#define SP1EMMC_PIN_RST   40u   // P1.08, active low
#define SP1EMMC_PIN_VCCQ  14u   // P0.14, I/O rail gate
...
#define CLK_HIGH()   nrf_gpio_pin_set(SP1EMMC_PIN_CLK)
#define CLK_LOW()    nrf_gpio_pin_clear(SP1EMMC_PIN_CLK)
#define CMD_HIGH()   nrf_gpio_pin_set(SP1EMMC_PIN_CMD)
#define CMD_LOW()    nrf_gpio_pin_clear(SP1EMMC_PIN_CMD)
#define DAT0_HIGH()  nrf_gpio_pin_set(SP1EMMC_PIN_DAT0)
#define DAT0_LOW()   nrf_gpio_pin_clear(SP1EMMC_PIN_DAT0)
#define DAT0_IN()    nrf_gpio_cfg_input(SP1EMMC_PIN_DAT0, NRF_GPIO_PIN_PULLUP)
// DAT0 as a HIGH-DRIVE output (H0H1) so edges are fast and clean.
#define DAT0_OUT()   nrf_gpio_cfg(SP1EMMC_PIN_DAT0, NRF_GPIO_PIN_DIR_OUTPUT, \
    NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, \
    NRF_GPIO_PIN_H0H1, NRF_GPIO_PIN_NOSENSE)
#define CMD_IN()     nrf_gpio_cfg_input(SP1EMMC_PIN_CMD, NRF_GPIO_PIN_PULLUP)
#define CMD_OUT()    nrf_gpio_cfg_output(SP1EMMC_PIN_CMD)
#define READ_CMD()   nrf_gpio_pin_read(SP1EMMC_PIN_CMD)
#define READ_DAT0()  nrf_gpio_pin_read(SP1EMMC_PIN_DAT0)

#define P0_CLK_BIT   (1u << SP1EMMC_PIN_CLK)
#define P0_DAT_BIT   (1u << SP1EMMC_PIN_DAT0)
...
#define RDAT_GET()   ((NRF_P0->IN >> SP1EMMC_PIN_DAT0) & 1u)
...
#define RST_ASSERT()   nrf_gpio_pin_clear(SP1EMMC_PIN_RST)
#define RST_RELEASE()  nrf_gpio_pin_set(SP1EMMC_PIN_RST)
#define VCCQ_ON()      nrf_gpio_pin_set(SP1EMMC_PIN_VCCQ)
#define VCCQ_OFF()     nrf_gpio_pin_clear(SP1EMMC_PIN_VCCQ)

static inline void sp1emmc_pins_init(void) {
    nrf_gpio_cfg(SP1EMMC_PIN_CLK, NRF_GPIO_PIN_DIR_OUTPUT, NRF_GPIO_PIN_INPUT_DISCONNECT,
        NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_H0H1, NRF_GPIO_PIN_NOSENSE);  // high-drive CLK
    nrf_gpio_cfg_output(SP1EMMC_PIN_CMD);
    DAT0_OUT();                                                          // high-drive DAT0
    nrf_gpio_cfg_output(SP1EMMC_PIN_RST);
    ...
    nrf_gpio_cfg_output(SP1EMMC_PIN_VCCQ);
}

static inline void sp1emmc_pins_release(void) {
    nrf_gpio_cfg_default(SP1EMMC_PIN_CLK);
    nrf_gpio_cfg_default(SP1EMMC_PIN_CMD);
    nrf_gpio_cfg_default(SP1EMMC_PIN_DAT0);
    nrf_gpio_cfg_default(SP1EMMC_PIN_RST);
...
#define SP1EMMC_TX_FRAME  ((uint8_t *)SPIM3_BUFFER_RAM_START_ADDR)
...
#define SP1EMMC_SPIM_TIMEOUT_TICKS  ((EMMC_TICKS_HZ * 20u) / 1000u)
...
static inline void sp1emmc_spim_init(void) {
    NRF_SPIM3->ENABLE = 0;
    NRF_SPIM3->PSEL.SCK = SP1EMMC_PIN_CLK;
...
static inline void sp1emmc_spim_deinit(void) {
    NRF_SPIM3->ENABLE = 0;
...
// built in sp1emmc_tx_frame() above.
static inline void sp1emmc_spim_xfer(const uint8_t *tx, uint32_t txlen, uint8_t *rx, uint32_t rxlen) {
    NRF_SPIM3->PSEL.MOSI = tx ? SP1EMMC_PIN_DAT0 : 0xFFFFFFFFu;
    NRF_SPIM3->PSEL.MISO = rx ? SP1EMMC_PIN_DAT0 : 0xFFFFFFFFu;
    NRF_SPIM3->ENABLE = 7;
    ...
    NRF_SPIM3->TASKS_START = 1;
    {
        ...
        while (!NRF_SPIM3->EVENTS_END) {
            if (ticks_since_raw(t0) >= SP1EMMC_SPIM_TIMEOUT_TICKS) {
