...
void common_hal_audiobusio_i2sout_construct(audiobusio_i2sout_obj_t *self,
    ...
    if (external_clock) {
        // External clock mode: SCK and LRCK are driven by something else.
    ...
    // MCK generation is only meaningful in internal clock mode; MCKFREQ and RATIO are
    // ignored by the peripheral when MODE = external_clock.
...
void common_hal_audiobusio_i2sout_deinit(audiobusio_i2sout_obj_t *self) {
    ...
    if (self->external_clock) {
        // Disconnect the input buffers construct() connected, as
        // nrfx_i2s_uninit() does.
...
void common_hal_audiobusio_i2sout_play(audiobusio_i2sout_obj_t *self,
    ...
    if (self->external_clock) {
        // The frame rate is whatever the external word select line runs at;
