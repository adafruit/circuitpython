...
//|
//|           As a clock follower ``bit_clock`` and ``word_select`` do not have to be sequential
...
//|           Only valid together with ``clock_follower``.
//|
...
static mp_obj_t audioi2sin_i2sin_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    enum { ARG_bit_clock, ARG_word_select, ARG_data, ARG_main_clock,
           ...
           ARG_clock_follower, ARG_invert_bit_clock };
    ...
    // is legal.
    const mcu_pin_obj_t *bit_clock = clock_follower
    ...
    const mcu_pin_obj_t *word_select = clock_follower
    ...
    common_hal_audioi2sin_i2sin_construct(self, bit_clock, word_select, data, main_clock,
        ...
        clock_follower, invert_bit_clock);
