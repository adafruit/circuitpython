...
# Play each stereo WAV file through both DAC channels simultaneously.
# Left channel → A0 (PA04, DAC_CH1), Right channel → A1 (PA05, DAC_CH2).
dac = audioio.AudioOut(board.A0, right_channel=board.A1)
for filename in sorted(samples):
    # Only play stereo files to exercise the right channel path.
    if "stereo" not in filename:
        continue
